# Hi 👋, I'm Arin Sarker
### MERN Stack Developer | Life-Long Learner
My Portfolio - https://my-portfolio-rouge-seven-ethmazk2wn.vercel.app/

<p align="center">
  <img width="1296" height="517" alt="banner" src="https://github.com/user-attachments/assets/56bc4c03-3ff5-40a0-b993-f08cfd23af3f" />
</p>

<p align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=45A1FF&center=true&vCenter=true&width=500&lines=Hi+I'm+Arin+Sarker;MERN+Stack+Developer;Building+Full-Stack+Web+Apps" alt="Typing SVG" />
  </a>
</p>

---

### 👨‍💻 About Me
"I am a passionate MERN Stack Developer who loves building scalable, responsive, and user-centric full-stack web applications. Beyond coding, I am always exploring new technologies and optimizing workflows."

- 📍 **Location:** Bangladesh
- 📧 **Email:** arinsarker07@gmail.com
- 💼 **Open to:** Full-Time Roles, Freelance Projects, and Collaborations

---

### 🚀 Current Activities
- 🔭 Working on: **Advanced MERN Stack E-commerce Project**
- 🌱 Learning: **Next.js, Framer Motion & Advanced Backend Optimization**
- 💬 Ask me about: **React, Node.js, Express, MongoDB, and Tailwind CSS**

---

### 🛠 Tech Stack & Skills

#### 🌐 Frontend Development
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=html,css,tailwind,js,react,nextjs,vite" />
  </a>
</p>
<ul>
  <li><strong>Core:</strong> HTML5, CSS3, JavaScript (ES6+)</li>
  <li><strong>Frameworks & Libraries:</strong> React.js, Next.js</li>
  <li><strong>Styling:</strong> Tailwind CSS, Bootstrap</li>
</ul>

---

#### 🗄️ Backend & Database
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=nodejs,express,mongodb,firebase" />
  </a>
</p>
<ul>
  <li><strong>Runtime & Framework:</strong> Node.js, Express.js</li>
  <li><strong>Database & ORM:</strong> MongoDB, Mongoose</li>
  <li><strong>Authentication & Security:</strong> Firebase Auth, JWT</li>
  <li><strong>API Architecture:</strong> REST API</li>
</ul>

---

#### 🔧 Tools & Deployment
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=git,github,vscode,postman,vercel,netlify,npm" />
  </a>
</p>
<ul>
  <li><strong>Version Control & Editors:</strong> Git, GitHub, VS Code</li>
  <li><strong>Testing & Debugging:</strong> Postman, Chrome DevTools</li>
  <li><strong>Deployment & Build Tools:</strong> Vercel, Netlify, Vite, npm</li>
</ul>

---

#### 🤖 AI-Driven Workflow
<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=chatgpt" />
  </a>
</p>
<ul>
  <li><strong>AI Assistants & IDEs:</strong> Gemini, GitHub Copilot, Cursor</li>
  <li><strong>Methodologies:</strong> Prompt Engineering, AI-assisted Debugging</li>
  <li><strong>Automation:</strong> README & Test-case Generation</li>
</ul>

---

#### 🤝 Soft Skills
<ul>
  <li>🧩 Problem Solving & Self-Learning</li>
  <li>🗣️ Team Communication & Feedback Handling</li>
  <li>📝 Technical Documentation</li>
  <li>⏳ Deadline Ownership</li>
</ul>

---

### 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=arinsarker07-oss&theme=react-dark&bg_color=0d1117&hide_border=true" alt="Arin's GitHub Activity Graph" width="100%" />
</p>
<p align="center">
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white" />
  <img src="https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white" />
  <img src="https://img.shields.io/badge/React.js-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.dot.js&logoColor=white" />
</p>

---

### 🌐 Connect with Me
<p align="left">
  <a href="https://www.linkedin.com/in/arin-sarker/" target="blank"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
  <a href="https://web.facebook.com/arin57276" target="blank"><img src="https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white" /></a>
</p>

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=arinsarker07-oss&label=Profile%20views&color=0e75b6&style=flat" alt="Views" />
</p>
